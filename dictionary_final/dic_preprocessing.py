...
glob_dict_path = "/profession/[14Spring]-Natural Language Processing/assignment2/dictionary-modified2.xml"
...
def parse_dictionary(dict_path):
    ...
    for lexeme in root:
        ...
        target_word = lexeme.attrib["item"].split(".")[0]
        for sense in lexeme:
            ...
            sense_dict["gloss"] = lemma_stem_sentence(sense.attrib["gloss"],target_word)
            examples = sense.attrib["examples"].split("|")
            example_list = []
            sense_dict["examples"] = example_list
            for sentence in examples:
                example_list.append(lemma_stem_sentence(sentence,target_word))
            """ Add wordnet
            """
            for wn_id in wordnet_list:
                wn_key = lexeme.attrib["item"]+"."+str(wn_id)

                synset = wn.synset(wn_key)
                sense_dict["gloss"] += lemma_stem_sentence(synset.definition, target_word)
                for sentence in synset.examples: 
                    sense_dict["examples"].append(lemma_stem_sentence(sentence, target_word))
            if "#" in sense_id:
                ids = sense_id.split("#")
                for s_itm in senses:
                    if s_itm["id"] in ids:
...
def parse_validation(validation_path):
    ...
    with open(validation_path) as f:
        for line in f:
            ...
            target_word = strs[0].strip().split(".")[0]
            ...
            for single_word in single_words:
                if single_word == "%%":
                    ...
                    continue
                lowered = single_word.lower()
                if lowered in glob_Lucene:
                    continue

                lemmed = lmtzr.lemmatize(single_word)
                stemmed = ls.stem(lemmed)
                
                if stemmed == target_word:
                    continue
...
def lemma_stem_sentence(sentence, target_word):
    lmtzr = WordNetLemmatizer()
    ls = LancasterStemmer()
    word_list = []
